...
export default memo(({ node }: { node: EntityNode }) => {
  ...
  const router = useRouter(); // CONZEPT PATCH

  // CONZEPT PATCH
  const loadArticle = ( label, language, qid ) => {

    const conzept_url = '/app/wikipedia/?t=' + label + '&l=' + language + '&type=wikipedia-qid&qid=' + qid ;

    window.parent.postMessage({ event_id: 'handleClick', data: { type: 'link', title: node.label, language: language, url: conzept_url, current_pane: 'ps1', target_pane: 'ps2' } }, '*' );
      ...
      <ThemedNodeInner>
        ...
        <ThemedContent className="content" hasSecondLabel={hasSecondLabel}>
          <div className={!hasLabelOnly ? "line-clamp" : ""}>
            {node.isRoot ? (
              <h1
                ...
                onKeyPress={() => loadArticle( node.label, settings.languageCode, node.id ) } // CONZEPT PATCH
                onClick={() => loadArticle( node.label, settings.languageCode, node.id ) } // CONZEPT PATCH
          ...
          { true && (
            <a
              //variant="light"
              className="show-tree-from-here"
              title="show tree from here"
              onClick={() => {
                //dispatch(setLoadingEntity(true));

                //console.log( node, currentProp?.slug, node.id );
