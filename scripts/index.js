    ...
    if ( !valid( topicResults.results ) ){
      ...
      $.each( topicResults.results, function( i, obj ){
        ...
        if ( valid( obj.id ) ){
            ...
            doc_url = 'https://www.inaturalist.org/taxa/' + obj.id;
	  ...
	  desc += `<div>${ obj.preferred_common_name }</div>`;

	}

        if ( valid( obj.matched_term ) ){

	  desc += `<div>${ obj.matched_term }</div>`;

	}

        if ( valid( img_attribution ) ){

	  desc += `<div>${ img_attribution }</div>`;
