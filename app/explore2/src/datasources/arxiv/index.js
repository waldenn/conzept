...
function autocompleteArxiv( results, dataset ){
  ...
  const total = $(xml).find('opensearch\\:totalResults').text() || 0;

  console.log( 'arxiv total: ', total );
          ...
          if ( Array.isArray( obj.authorships ) ){
            ...
            $.each( obj.authorships, function ( j, a ) {
                ...
                if ( valid( a.author.id ) ){
                  ...
                  let author_id = a.author.id.replace('https://arxiv.org/', '');

                  author_url = `https://arxiv.org/works?sort=cited_by_count%3Adesc&column=display_name,publication_year,type,open_access.is_oa,cited_by_count&page=1&filter=authorships.author.id%3A${ author_id }`;
                ...
                // FIXME: the link does not work in Arxiv-in-presentation-mode
          ...
          if ( Array.isArray( obj.concepts ) ){
            ...
            $.each( obj.concepts, function ( j, concept ) {
                ...
                if ( valid( concept.wikidata ) ){
                  ...
                  // FIXME: this does not work in Arxiv-in-presentation-mode
