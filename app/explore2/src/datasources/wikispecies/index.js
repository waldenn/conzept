...
function autocompleteWikispecies( results, dataset ){

  const source = 'wikispecies';
...
function processResultsWikispecies( topicResults, struct, index ){
  ...
  return new Promise(( resolve, reject ) => {
    ...
    if ( !valid( topicResults?.query?.search ) ){
      ...
      $.each( topicResults.query.search, function( i, obj ){
        ...
        let url         = valid( title )? `https://species.m.wikimedia.org/wiki/${title}?uselang=${explore.language}` : '';
...
function resolveWikispecies( result, renderObject ){
...
function renderMarkWikispecies( inputs, source, q_, show_raw_results, id ){
