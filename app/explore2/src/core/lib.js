...
function doGeospatialSearch( url, custom ){
  ...
  explore.custom  = custom; // 'lat;lon;radius;country'
  explore.uri     = url;
    ...
    let topic_html = 
      '<ul class="multi-value mv-select-card" name="' + args.target + '"><li>' +
        ...
        '<span class="mv-extra-buttons">' +
          ...
          '<a href="javascript:void(0)" class="mv-extra-icon" title="streaming video" aria-label="streaming video" role="button"' + setOnClick( Object.assign({}, args, { type: 'wander', title: encodeURIComponent( new_title ), qid: '', language  : explore.language } ) ) + '"> <span class="icon"><i class="fa-solid fa-tv" style="position:relative;"></i></span></a>' +
  ...
  */
  ...
  if ( action.startsWith( 'command-' ) ){ // command API action
    ...
    handleClick({ 
      ...
      language  : explore.language,
      qid       : '',
      url       : `https://${explore.host}/app/video/?l=${explore.language}#/search/${ encodeURIComponent( setQuotes( topics ) ) }`,
      tag       : '',
      languages : '',
      custom    : '',
      target_pane : 'p1',
    ...
    handleClick({ 
      ...
      language  : explore.language,
      qid       : '',
      url       : `https://openlibrary.org/search?q=${ encodeURIComponent( setQuotes( topics ) ) }&language=${explore.lang3}`,
      tag       : '',
      languages : '',
      custom    : '',
      target_pane : 'p1',
        ...
        let more_html = 
          '<ul class="catmore multi-value" name="' + args.target + '">' +
            '<li><span class="mv-extra-buttons noindent">' +
              ...
              ( explore.isMobile ? '' : '<a href="javascript:void(0)" class="mv-extra-icon" title="streaming video" aria-label="streaming video" role="button"' + setOnClick( Object.assign({}, args, { type: 'wander', title: title, qid: '', language  : explore.language } ) ) + '"> <span class="icon"><i class="fa-solid fa-tv" style="position:relative;"></i></span></a>' ) +
...
function setupSpeechRecognition(){
	...
	if ( SpeechRecognition ){
		...
		explore.recognition.continuous	= false;
		...
		function resultOfSpeechRecognition(event){
  		...
  		$('#srsearch').val('');
			...
			var transcript = event.results[ event.resultIndex ][0].transcript; // latest spoken text
        ...
        spoken_text = spoken_text.charAt(0).toLowerCase() + spoken_text.slice(1);

			}

			if ( spoken_text.endsWith('.') ){ // remove ending "."

     		spoken_text = spoken_text.slice(0, -1);
