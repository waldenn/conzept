...
function convert_coordinates_WGS84_to_EPSG_28992( lat, lon, pos ){
  ...
  const r = proj4( 'WGS84', 'EPSG:28992', [ parseFloat( lon ), parseFloat( lat ) ] );
...
function setQuotes( term, showComma ){ // used for the correct quoting of bookmark-based term searches

  showComma = valid( showComma )? true : false;

  if ( Array.isArray( term ) ){ // list of terms

    if ( showComma ){
      term = term.map( term => `%22${term.trim()}%22`).join(', ');
    }
    else {
      term = term.map( term => `%22${term.trim()}%22`).join(' ');
    }
  ...
  else { // (comma-delimited) term-string

    if ( term.includes(',') ){ // likely multiple terms were entered (resulting from a bookmark action)

      // split them
      const list = term.split(',');

      // join them again with quotes

      if ( showComma ){
        term = list.map( term => `%22${term.trim()}%22`).join(', ');
      }
      else {
        term = list.map( term => `%22${term.trim()}%22`).join(' ');
      }
    ...
    else if ( !term.includes('"') ){ // add quotes around complete term
