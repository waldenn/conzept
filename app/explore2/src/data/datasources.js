const datasources = {
  ...
  'wikipedia': {
    ...
    sort_map:               { // https://www.mediawiki.org/wiki/Help:CirrusSearch
                              ...
                              'update-desc'     : 'last_edit_asc',
                              'update-asc'      : 'last_edit_desc',
                              'random'          : 'random',
  ...
  'archive': {
    ...
    url:                    '${datasources.archive.endpoint}?sort[]=${sortby}&rows=${datasources.archive.pagesize}&output=json&page=${explore.page}&language%3A"${ wp_languages[ explore.language ].name }"&q=${term}',
  ...
  'arxiv': {
    ...
    url:                    '${datasources.arxiv.endpoint}?search_query=all:${term}&sortBy=${sortby}&sortOrder=${getSortDirection( source )}&start=${ ( explore.page - 1) * datasources.arxiv.pagesize }&max_results=${datasources.arxiv.pagesize}&sortBy=relevance&sortOrder=descending',
  ...
  'rijksmuseum': {
    ...
    url:                    '${datasources.rijksmuseum.endpoint}/${ selectLanguageFrom( ["nl", "en"], "en" ) }/collection?key=8MT2OLE3&imgonly=true&s=${sortby}&p=${explore.page}&ps=${datasources.rijksmuseum.pagesize}&culture=${ selectLanguageFrom( ["nl", "en"], "en" ) }&q=${ removebracesTitle(term) }',
