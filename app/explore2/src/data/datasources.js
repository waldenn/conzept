const datasources = {
  ...
  'europeana': {
    ...
    autocomplete_url:       '${datasources.europeana.endpoint}/record/search.json?wskey=4ZViVZKMe&view=grid&query=${term}&media=true&sort=${ valid( sortby )? sortby : "score+desc" }&profile=standard&rows=${datasources.europeana.datasources.europeana.autocomplete_limit}&start=1&${ valid( filterby ) && Object.values( d.filter_map ).includes( filterby ) ? "qf=" + filterby : "qf=" }${ valid( [ datemin, datemax ] )? "&qf=timestamp_created:%5B" + datemin + "+TO+" + datemax + "%5D" : "" }',
    autocomplete_format:    'json',
    autocomplete_connect:   'json',
    autocomplete_limit:     5,
  },
