...
export function getTrueAnomaly(eccentricAnomaly, eccentricity) {
    ...
    let trueAnomaly = Math.atan2(Math.sin(eccentricAnomaly) * Math.sqrt(1 - eccentricity * eccentricity), Math.cos(eccentricAnomaly) - eccentricity);
...
export function getPerifocalToCartesianMatrix(argumentOfPeriapsis, inclination, rightAscension) {
    let res = new Mat3();
    let cosap = Math.cos(argumentOfPeriapsis), sinap = Math.sin(argumentOfPeriapsis), cosi = Math.cos(inclination), sini = Math.sin(inclination), cosraan = Math.cos(rightAscension), sinraan = Math.sin(rightAscension);
