...
const tagsWithMandatoryTypePosition = new Set([// These both show curly brackets in the doc signature and examples
// "typeExpression"
'implements', // "typeName"
'type']);
const tagsWithMandatoryTypePositionClosure = new Set([...tagsWithMandatoryTypePosition, 'this', 'define']); // All of these have a signature with "type" except for
//  `augments`/`extends` ("namepath")
//  `param`/`arg`/`argument` (no signature)
//  `property`/`prop` (no signature)
// `modifies` (undocumented)

const tagsWithOptionalTypePosition = new Set([// These have the example showing curly brackets but not in their doc signature, e.g.: https://jsdoc.app/tags-enum.html
...
'modifies']);
const tagsWithOptionalTypePositionClosure = new Set([...tagsWithOptionalTypePosition, 'export', // Shows the signature with curly brackets but not in the example
// "typeExpression"
...
const namepathDefiningTags = new Set([// These appear to require a "name" in their signature, albeit these
...
const tagsWithOptionalNamePosition = new Set([...namepathDefiningTags, // `borrows` has a different format, however, so needs special parsing;
...
const tagMightHaveTypePosition = (mode, tag) => {
  if (mode === 'closure') {
    return tagsWithMandatoryTypePositionClosure.has(tag) || tagsWithOptionalTypePositionClosure.has(tag);
  }

  return tagsWithMandatoryTypePosition.has(tag) || tagsWithOptionalTypePosition.has(tag);
};
