...
wikibase.queryService.ui.ResultView = ( function( $, download, window ) {
    ...
    while ( ( matchMeta = regexMeta.exec( this._query ) ) !== null ) {

        var meta_values = matchMeta[1].split(':') || [];
        //console.log( meta_values );

        var title_ = meta_values[0] || '';
        title_ = title_.split('#defaultView')[0]; // ugly hack to remove this potential string from the title

        var layout_ = meta_values[1] || '';
        //console.log('meta layout: ', layout_ );

        if ( layout_ === 'layout-topdown' ){
          self.window.click_topdown_layout = true;
        }
        else {
          self.window.click_topdown_layout = false;
        }

        //console.log('window.layout: ', window.layout_ );
        //console.log('window.click_topdown_layout: ', window.click_topdown_layout );

        //console.log('meta title: ', title_ );

        $('#queryTitle').text( title_ );
        //$('#' + window.layout_ ).trigger( 'click' );
