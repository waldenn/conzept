...
wikibase.queryService.ui.resultBrowser.helper.FormatterHelper = ( function( $, moment ) {
	...
	SELF.prototype.formatRow = function( row, embed ) {
      ...
      // CONZEPT PATCH START
      var el = '<div id="key-' + key.toLowerCase() + '" class="wdq-row">';
      $result.prepend( $( el ).append( self.formatValue( value, key, embed ) ) );
      // CONZEPT PATCH END
	...
	SELF.prototype.formatValue = function( data, title, embed ) {
      ...
      //console.log( data );
      var qid = value.replace('http://www.wikidata.org/entity/Q', '');
      ...
      if ( Number.isInteger( parseInt( qid )) ){
        ...
        var fid = iframe_in_jquery.attr('id');
        var cid = iframe_in_jquery.hasClass('querybuilder__result__iframe');

        if ( iframe_in_jquery.hasClass('querybuilder__result__iframe') ){ // click from sidebar TODO: is this being used?

          mesg = JSON.stringify( { event_id: 'handleClick', data: { type: 'wikipedia-qid', title: '', hash: '', language: l, qid: qid, current_pane: getCurrentPane(), target_pane: getTargetPane() } } );
          console.log('is not used?', getTargetPane() );

        }
        else { // click from content-pane
        ...
        $link = $( '<a>' ).attr( { title: title, href: 'javascript:void(0)', onclick: 'window.top.postMessage( ' + mesg + ' , "*" )', class: 'item-link', rel: '' } );

      }
      else {

        $link = $( '<a>' ).attr( { title: title, href: value, target: '_blank', class: 'item-link', rel: 'noopener' } );
          ...
          // CONZEPT PATCH START
          var media_url = 'https://commons.wikimedia.org/wiki/Special:FilePath/' + encodeURIComponent( this.getCommonsResourceFileName( value ) );

          if ( media_url.endsWith('.ogg') || media_url.endsWith('.oga') || media_url.endsWith('.mp3') || media_url.endsWith('.wav') ){

            $html.prepend( '<audio controls><source src="' + media_url + '"></audio>' );

          }
          else if ( media_url.endsWith('.ogv') || media_url.endsWith('.webm') ){ // render as video

            $html.prepend( '<video width="100%" controls><source src="' + media_url + '"></video>' );

          }
          // TODO images
          // ...
          else { // some other media format (PDF, ...)
