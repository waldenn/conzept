...
function markupEntities( text ){
  ...
  // Matches consecutive capitalized words, e.g. "New York" or "John Smith"
  const entityRegex = /[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*/g;

  return text.replace(entityRegex, match => { // Capitalize the first letter of each word and add entity tags

    const words = match.split(' ');
    const capitalizedWords = words.map(word => word.charAt(0).toUpperCase() + word.slice(1));
    const entityName = capitalizedWords.join(' ');

    return `<a href="javascript:void(0)" title="" onclick="exploreString( &quot;${encodeURIComponent( entityName )}&quot;)">${entityName}</a> `;
